...
internal class BinTreeNodeReader
{
    ...
    protected byte[] readBytes(int token)
    {
        ...
        {
            ret = Encoding.UTF8.GetBytes(this.getToken(token));
        }
        else if (token == 0)
        {
            ret = new byte[0];
        ...
        else if (token == 0xfe)
        {
            ...
            ret = Encoding.UTF8.GetBytes(this.getToken(tmpToken + 0xf5));
        }
        else if (token == 0xfa)
        {
            string user = Encoding.UTF8.GetString(this.readBytes(this.readInt8()));
            string server = Encoding.UTF8.GetString(this.readBytes(this.readInt8()));
            if ((user.Length > 0) && (server.Length > 0))
            {
                ret = Encoding.UTF8.GetBytes(user + "@" + server);
            }
            else if (server.Length > 0)
            {
                ret = Encoding.UTF8.GetBytes(server);
            }
        }
        return ret;
    }
    ...
    protected IEnumerable<KeyValue> readAttributes(int size)
    {
        ...
        for (int i = 0; i < attribCount; i++)
        {
            ...
            string key = Encoding.UTF8.GetString(keyB);
            string value = Encoding.UTF8.GetString(valueB);
    ...
    protected ProtocolTreeNode nextTreeInternal()
    {
        ...
        string tag = Encoding.UTF8.GetString(this.readBytes(token2));
